...
MQTT + REST API를 통해 RPi의 `ugv_bridge` 노드와 통신하며, 브라우저에서 로봇 모니터링과 제어가 가능합니다.
...
- **자율주행** — 맵 클릭으로 Nav2 목표 전송, 경로 표시, 취소
...
└──────┬─────────────────────┬─────────────────┘
       │ MQTT (ws://1884)    │ REST (http://8080)
       │ 실시간 센서 구독     │ 명령 전송 / 맵 조회
┌──────▼─────────────────────▼─────────────────┐
│  ugv_bridge (ugv_ws 레포)                     │
│  ROS 2 ↔ MQTT/REST 브릿지                    │
...
|--------|------|
| Frontend | Vue 3 + Vite |
| 실시간 통신 | MQTT.js → ws://RPi:1884 (Mosquitto WebSocket) |
| 명령/조회 | fetch → http://RPi:8080 (FastAPI REST API) |
...
- Mosquitto 브로커 (RPi, 포트 1883/1884)
...
브라우저에서 MQTT 브로커 URL (`ws://192.168.0.71:1884`) 확인 후 Connect.
